...
<div class="flex flex-col px-3 pb-4 rounded-lg">
  {#if mediaItems.length > 0}
    <Splide
      ...
      class="h-[min(calc(100vh-15rem),45rem)] relative rounded-2xl"
    >
      <SplideTrack>
        {#each mediaItems as item, index}
          <SplideSlide
            ...
            class={cx(
              `h-[min(calc(100vh-20rem),45rem)] p-10 min-w-0 relative video-slide-${index} max-h-full transition-all`,
            )}
          >
            <div
              class={cx(
                "flex justify-center h-full max-h-full duration-550 slide-content ease-[cubic-bezier(0.8,0,0,0.8)]",
              )}
            >
              {#if item.type === "image"}
                <img
      ...
      <div
        class="flex absolute top-0 left-0 flex-col justify-end px-1 w-full h-full pointer-events-auto splide__arrows"
      >
        <div
          class="relative w-full bg-gradient-to-r rounded-2xl translate-y-30 h-35 from-black/70 to-black/70"
        >
          <button
            class="absolute !left-[1rem] !bg-slate-950/20 hover:!bg-slate-900/30 !backdrop-blur-lg !border-1 !border-slate-800/80 !transition-all !opacity-100 z-10 !rounded-lg !w-13 !h-28 splide__arrow splide__arrow--prev"
          >
            <Fa class="text-slate-400" icon={faArrowRight}></Fa>
          </button>
          <div class="flex w-full max-w-full h-full track-mask">
            <Splide
              class="flex justify-center items-center w-full h-full thumbnail-carousel"
                ...
                >
                  {#if item.type === "image"}
                    <img
                      ...
                      src={item.src}
                      class="object-cover w-full h-full transition-all cursor-pointer brightness-70 aspect-square hover:!brightness-100"
                    />
                  {:else}
                    <div
                      class="flex justify-center items-center rounded-lg border-2 backdrop-blur-2xl transition-opacity cursor-pointer aspect-square border-slate-800/40 hover:opacity-100 bg-black/30"
                    >
                      <Fa
                        class="w-full text-3xl text-slate-300"
                        icon={faPlayCircle}
                      ></Fa>
                    </div>
                  {/if}
          ...
          <button
            class="absolute !right-[1rem] !bg-slate-950/20 hover:!bg-slate-900/30 !backdrop-blur-lg !border-1 !border-slate-800/80 !transition-all !opacity-100 z-10 !rounded-lg !w-13 !h-28 splide__arrow splide__arrow--next"
          >
            <Fa class="text-slate-400" icon={faArrowRight}></Fa>
          </button>
        </div>
