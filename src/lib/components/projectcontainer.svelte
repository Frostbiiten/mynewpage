<script>
  ...
  function wordFilter(project, word) {
    ...
    let tagNameFound = project.tags
      .map((t) => t.toLowerCase())
      .some((str) => str.includes(word));
    let tagFilter = tags
      .map((t) => t.toLowerCase())
      .every((tag) => project.tags.map((t) => t.toLowerCase()).includes(tag));
  ...
  function getImage(imgName) {
    if (images[`/src/lib/img/projects/${imgName}`]) {
      return {
        img: images[`/src/lib/img/projects/${imgName}`].default,
        found: true,
      };
    }
    return {
      img: images[`/src/lib/img/projects/null.png`].default,
      found: false,
    };
  }
  ...
  function scaleY(node, { start = 0, duration = 300, easing = cubicOut }) {
    return {
      ...
      easing,
      css: (t) =>
        `transform: scaleY(${start + (1 - start) * t}); opacity: ${t}`,
    };
  }
  ...
  function scaleX(node, { start = 0, duration = 300, easing = cubicOut }) {
    return {
      ...
      easing,
      css: (t) =>
        `transform: scaleX(${start + (1 - start) * t}); opacity: ${t}`,
    };
  }
...
<div class="space-y-3">
  <div class="flex flex-row gap-3 bg-zinc-950/95">
    <!-- perhaps add lg:basis-1/2 too? -->
    <div
      class="flex flex-row flex-shrink justify-center items-center pb-3 basis-full"
    >
        ...
        >
          <div class="grid grid-cols-1 grid-rows-1">
            {#if tag.toLowerCase().includes("won")}
              <Fa
                class="col-span-1 col-start-1 row-span-1 row-start-1 text-sky-500 group-hover:text-transparent"
                icon={faTrophy}
              />
            {:else}
  ...
  <div class="grid grid-cols-1 gap-3 w-full h-full lg:grid-cols-2">
    {#each pj_test.projects
      .filter(inSearch)
      .sort(sorter) as project, index (project.name)}
      <div
