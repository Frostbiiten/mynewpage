...
fn mainFragment(
  ...
  let o = kawaseBlurUniforms.uOffset;
  let gamma = kawaseBlurUniforms.uGamma.x;
  let invGamma = kawaseBlurUniforms.uGamma.y;

  let c0 = toLinearPremul(textureSample(uTexture, uSampler, uv - o), gamma);
  let c1 = toLinearPremul(textureSample(uTexture, uSampler, uv + o), gamma);
