...
import fragment from "./kawase-blur.frag?raw";
import source from "./kawase-blur.wgsl?raw";
import fragmentClamp from "./kawase-blur-clamp.frag?raw";
import sourceClamp from "./kawase-blur-clamp.wgsl?raw";
...
export class KawaseBlurFilter extends Filter {
  ...
  constructor(
    ...
    super({
      ...
      resources: {
        kawaseBlurUniforms: {
          ...
          uGamma: { value: new Float32Array(2), type: "vec2<f32>" },
        },
      },
    });
    ...
    const g = Math.max(0.0001, options.gamma ?? 2.2);
    this.uniforms.uGamma[0] = g;
    this.uniforms.uGamma[1] = 1.0 / g;
  ...
  get gamma(): number {
    return this.uniforms.uGamma[0];
  }
  set gamma(v: number) {
    const g = Math.max(0.0001, v);
    this.uniforms.uGamma[0] = g;
    this.uniforms.uGamma[1] = 1.0 / g;
  }
  ...
  private _updatePadding() {
    this.padding = Math.ceil(
      this._kernels.reduce((acc, v) => acc + v + 0.5, 0),
    );
  }
