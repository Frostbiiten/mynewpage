<script>
  ...
  const categories = [
    {
      ...
      gallery: Object.values(
        import.meta.glob("$lib/img/creative/images/sketches/*", {
          eager: true,
        }),
    ...
    },
    {
      ...
      image: digital,
      gallery: Object.values(
        import.meta.glob("$lib/img/creative/images/digital/*", {
          eager: true,
        }),
      ).map((module) => module.default),
      contrast: "1.05",
    ...
    },
    {
      ...
      image: photos,
      gallery: Object.values(
        import.meta.glob("$lib/img/creative/images/photos/*", {
          eager: true,
        }),
      ).map((module) => module.default),
      contrast: "1.35",
    ...
    },
    {
      ...
      gallery: Object.values(
        import.meta.glob(
          "$lib/img/creative/images/misc/*.{png,jpg,jpeg,webp,gif,mp4,webm,mov}",
          {
            eager: true,
          },
        ),
  ...
  $effect(() => {
    ...
    const check = () => (isMd = wQuery.matches);
    check();
  ...
  function setCategory(newCategory) {
    if (category === "") {
      ...
      currentImages = categories.find((el) => {
        return el.name === newCategory;
      }).gallery;
    } else category = "";
  }

  let visibleCategories = $derived(
    !category ? categories : categories.filter((cat) => cat.name === category),
  );
  let loadedImg = $state(
    Object.fromEntries(categories.map((cat) => [cat.name, false])),
  );
  ...
  function scaleX(node, { delay = 0, duration = 400, easing = expoOut }) {
    return {
      ...
      css: (t) =>
        isMd
          ? `max-width: ${t * 25}%; overflow: hidden;`
          : `max-height: ${t * 25}%; overflow: hidden;`,
    };
  }
  ...
  function meAnim(node, { delay = 0, duration = 400, easing = expoOut }) {
    return {
      ...
      easing,
      css: (t) =>
        `width: calc(var(--spacing) * ${56 * t}); min-width: calc(var(--spacing) * ${56 * t});`,
    };
  }
...
<svelte:head>
  ...
  <meta
    name="description"
    content="This page contains my attempts at creative pieces"
  />
  <meta
    name="keyword"
    content="sketches, digital, photos, edits, drawings, 3d models"
  />
  <meta
    name="keywords"
    content="sketches, digital, photos, edits, drawings, 3d models"
...
<div
  class="flex flex-row justify-center -mt-8 -mb-8 w-full overflow-clip md:-mt-5 grow"
>
  <div class="px-8 space-y-5 min-h-full max-w-screen md:max-w-7xl grow w-7xl">
    <div
      class={cx(
        "flex flex-col md:flex-row transition-all items-stretch w-full h-full justify-stretch duration-300",
        visibleCategories.length != 1 && "gap-2",
        visibleCategories.length == 1 && "gap-0",
      )}
    >
      {#if !category}
        <div
          class="bg-[#d9d9d9] rounded-lg overflow-clip md:w-56 md:min-w-56 hidden md:block"
          in:meAnim={{ duration: 400 }}
          out:meAnim={{ duration: 400 }}
        >
          <div class="pt-4 pl-5 space-y-2 h-1/2">
            <h1 class="w-full leading-8 font-mono text-[225%] text-gray-950">
              creative works
            </h1>
          </div>
          <img
            src={me}
            alt="me"
            class="hidden object-[47%_0%] object-cover h-1/2"
          />
          <p class="hidden content-end p-6 pb-6 h-1/2 text-gray-900">
      ...
      {#each visibleCategories as cat, i (cat.name)}
        <button
          ...
          style="--pos: {cat.objectPosition ??
            '50% 50%'}; --hover-pos: {cat.hoverObjectPosition ??
            '35% 50%'}; --contr: {cat.contrast ?? '1.55'}"
        ...
        >
          <img
            class={cx(
              "object-cover opacity-0 rounded-lg contrast-[var(--contr)] pointer-events-none ease-[cubic-bezier(0.233,0.001,0,1.166)] duration-700 absolute top-0 left-0 md:h-full object-[var(--pos)] group-hover:object-[var(--hover-pos)]",
              cat.additionalImageClasses ?? "",
              loadedImg[cat.name] && "opacity-100",
            )}
            alt={cat.name}
            src={cat.image}
            onload={() => {
              loadedImg[cat.name] = true;
            }}
          />
          ...
          <div
            class={cx(
              "absolute transition-colors mix-blend-hard-light duration-100 from-gray-950/80 top-0 left-0 w-full h-full bg-gradient-to-t to-60%",
              visibleCategories.length != 1 &&
                "group-hover:from-[rgb(160,190,255)]",
              visibleCategories.length == 1 && "group-hover:from-rose-400/30",
            )}
          ></div>

          {#if visibleCategories.length == 1}
            <div
              class="flex absolute top-0 left-0 justify-center items-start w-full h-5 md:h-20"
            >
              <h2
                class="absolute hidden tracking-[0.15em] px-3 text-2xl text-gray-100 rounded-sm bg-gray-950"
              >
                {visibleCategories[0].name}
              </h2>
              <h2
                class="hidden absolute bg-[#d9d9d9] break-all pb-4 px-3 leading-8 text-[225%] text-gray-950 pt-4 text-left"
              >
                {visibleCategories[0].name}
              </h2>
            </div>
          ...
          <h3
            class={cx(
              "absolute flex items-center gap-3 text-lg tracking-widest group-hover:tracking-widest m-2 md:m-6 duration-300 ease-[cubic-bezier(0.0,1,0,1.0)] font-mono origin-bottom-left pointer-events-none group-hover:scale-[100%_300%] group-hover:translate-y-3 group-hover:uppercase group-hover:font-black ",
      ...
      <div
        class={cx(
          "md:w-0 h-0 md:h-full relative overflow-y-visible ease-[cubic-bezier(1,0,0,1)] duration-600",
          visibleCategories.length == 1 && "md:w-6/7 h-6/7",
        )}
      >
        {#if visibleCategories.length == 1}
          <div
          ...
          >
            <h2
              ...
              class={cx(
                "origin-top text-sm md:text-2xl md:origin-left",
                visibleCategories.length == 1 && "md:scale-x-100 scale-y-100",
                visibleCategories.length != 1 && "scale-y-400 md:scale-x-400",
              )}
            >
              {visibleCategories[0].name}
            </h2>
          ...
          >
            ...
            >
              <div class={cx("absolute max-h-[100%] w-full")}>
