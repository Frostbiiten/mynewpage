<script>
  ...
  function getImage(imgName) {
    if (images[`/src/lib/img/music/covers/${imgName}`]) {
      return {
        img: images[`/src/lib/img/music/covers/${imgName}`].default,
        found: true,
      };
    }
    return {
      img: images[`/src/lib/img/music/covers/null.png`].default,
      found: false,
    };
  }
  ...
  function checkScroll() {
    widenTracklist =
      tracklistScrollbox.scrollHeight > tracklistScrollbox.clientHeight;
  }
  ...
  function playNewSong() {
    currentVideoId =
      music_data.collections[playingCollection].tracklist[playingSong].link;
  ...
  function playNext() {
    let songCount = 0;
    if (music_data.collections[playingCollection]) {
      songCount = music_data.collections[playingCollection].tracklist.length;
    }
  ...
  function scrollToElement(el, options = {}) {
    ...
    function calcTargetScrollTop() {
      ...
      return (
        scrollOffset + offset - container.clientHeight / 2 + el.clientHeight / 2
      );
    }
    ...
    function animateScroll(currentTime) {
      ...
      container.scrollTop =
        currentScrollTop + (targetScrollTop - currentScrollTop) * easing(t);
  ...
  function animatePerspective() {
    ...
    children.forEach((child) => {
      ...
      const dx =
        (childCenterY - containerCenterY) / (containerRect.height / -2);
      //const translateX = (1-Math.cos(dx * Math.PI / 2)) * containerRect.height * 0.05;
      const translateY =
        Math.max(
          Math.min(Math.sign(dx) * Math.pow(Math.abs(dx), 3), Math.abs(dx * 2)),
          -Math.abs(dx * 2),
        ) *
  ...
  function updateSpeech(time) {
    ...
    const current = getPrev(
      music_data.collections[playingCollection]?.tracklist[playingSong]
        .hotspots,
      time,
    );
    const next = getNext(
...
<svelte:head>
  ...
  <meta
    name="description"
    content="This page contains some of my ramblings on media that has inspired me"
  />
...
<div class="flex flex-row justify-center items-center w-full">
  <div class="px-8 w-screen max-w-7xl pace-y-5">
    <div class="hidden relative w-40 h-70">
      <h1
        class="absolute top-0 left-[100%] text-[12rem] font-bold leading-none text-slate-200 rotate-90 align-text-top origin-top-left"
      >
        OK
      </h1>
    </div>
    ...
    <div
      class={cx(
        "p-0 md:scale-100 max-h-[calc(100vh+20rem)] min-[105rem]:max-h-[calc(100vh-20rem)] mt-3 w-0 flex items-start min-[105rem]:translate-x-[-14rem] ease-[cubic-bezier(0,1,0,1)] transition-all duration-1000",
        currentMenu === "music"
          ? "translate-y-0 pointer-events-auto w-full max-sm:-translate-x-5 max-sm:w-[calc(100%+2rem)] min-[105rem]:w-[calc(100%+14rem)] opacity-100 h-[calc(100vh+6rem)] "
          : "translate-y-30 pointer-events-none opacity-0 h-0",
      )}
    >
      <div
        class="flex flex-row justify-start -mr-2 h-20 rounded-lg border-2 md:mr-0 md:rounded-2xl border-slate-700 bg-gray-900/20 md:h-50"
      >
        <div
          class="relative h-[80vh] md:h-[calc(30vh+30rem)] w-20 md:w-50 self-center"
        >
          <div
            class="absolute bottom-[65%] w-full h-40 min-[105rem]:hidden"
          ></div>
          ...
          >
            ...
            {#each music_data.collections as collection, index (collection)}
              <button
                ...
                bind:this={collectionElements[collection.name]}
                onclick={() => {
                  //collectionElements[collection.name].scrollIntoView({"behavior": "smooth", "inline": "center", "block": "end"})
                  scrollToElement(collectionElements[collection.name], {
                    duration: 200,
                  });
                }}
              >
                <img
                  ...
                  src={getImage(collection.img).img}
                  alt={collection.name}
                />
              </button>
            {/each}
          ...
          <button
            ...
            class="inline-flex absolute gap-2 justify-center items-center pr-3 w-full top-198 z-9999 text-slate-600 hover:text-slate-500"
          >
      ...
      <div
        class="flex flex-col gap-3 pl-5 w-full font-mono max-w-[calc(100%+15rem)] md:max-w-[calc(100%-13.5rem)] min-[100rem]:w-full"
      >
        {#if currentCollection !== null}
          <div
            class="flex flex-col gap-2 p-5 pl-7 w-full bg-gray-900 rounded-xl md:h-35"
          >
            <p
              class="-mb-2 text-gray-500 md:text-lg max-lg:mt-4 max-[105rem]:mt-3"
            >
              {music_data.collections[currentCollection].artist}
            </p>
            <h2
              class="font-sans text-2xl font-bold sm:text-3xl md:text-[2.1rem] lg:md:text-[3rem] min-[105rem]:text-[4.25rem] text-slate-100"
            >
        ...
        >
          <div
            class="absolute hidden duration-400 left-0 top-0 w-full h-full bg-gradient-to-r ease-[cubic-bezier(0,1,0,1)] rounded-t-lg from-blue-950/40 via-50% via-blue-900/15 to-indigo-500/0"
          ></div>
          <div
            class="absolute duration-400 left-0 top-0 w-full h-full bg-gradient-to-r ease-[cubic-bezier(0,1,0,1)] rounded-t-lg bg-blue-950/15 border-2 border-b-0 border-slate-900"
          ></div>

          <p class="hidden text-lg sm:block">#</p>
          <p class="min-[105rem]:w-[27.85rem] w-[calc(100%-10rem)]">Name</p>
          <div class="block w-3 sm:hidden"></div>
          <p class="opacity-0 duration-200 sm:opacity-100">Fav</p>

          <p class="ml-[0.35rem] hidden sm:block">Len</p>
        </div>

        <div
          class="w-[120%] hidden ml-4 mt-3 h-1 bg-gradient-to-r from-blue-900/0 via-blue-950/70 via-7% to-blue-900/0 opacity-100"
        ></div>
        <div class="flex flex-row -mt-3">
          <div
            bind:this={tracklistScrollbox}
            style={`scrollbar-color: rgba(200, 200, 230, 0.4) transparent; --sw: ${scrollbarWidth}px`}
            class={cx(
              "flex text-xs md:text-base max-w-full overflow-y-auto overflow-x-visible flex-col md:gap-2 pr-6 pb-2 pt-3 pl-3 leading-tight whitespace-normal duration-400 ease-[cubic-bezier(0.12,0.639,0.34,1)] min-[105rem]:min-h-110 max-h-[calc(100vh-31.25rem)] rounded-bl-xl border-slate-900 border-2 border-r-0 border-t-0 w-full min-[105rem]:w-160",
              widenTracklist &&
                "pr-[calc(-var(--sw)*5)] mr-[calc(-var(--sw)*10)] min-[105rem]:w-[calc(40rem+var(--sw))]",
            )}
          >
            {#if currentCollection !== null}
              {#each music_data.collections[currentCollection].tracklist as song, index (song)}
                <button
                  ...
                  class={cx(
                    "flex group relative pl-[0.35rem] items-center w-full flex-row gap-[1.2rem] py-[0.35rem] cursor-pointer",
                  )}
                >
                  <div
                    class={cx(
                      "absolute group-hover:scale-x-100 group-hover:opacity-90 transition-all origin-left scale-x-0 -left-1 top-0 w-full h-full bg-gradient-to-r ease-[cubic-bezier(0,1,0,1)] rounded-md from-blue-950/40 via-50% via-blue-900/15 to-indigo-500/0 opacity-0 duration-1000",
                      playingSong == index &&
                        playingCollection == currentCollection &&
                  ...
                  <p
                    class={cx(
                      "transition-all hidden sm:block duration-200 ease-[cubic-bezier(1,0,0,1)]",
                      (playingSong != index ||
                        playingCollection != currentCollection) &&
                        "text-gray-600",
                      playingSong == index &&
                  ...
                  <p
                    class={cx(
                      "transition-all grow overflow-ellipsis overflow-hidden md:text-nowrap text-left duration-200 ease-[cubic-bezier(1,0,0,1)]",
                      (playingSong != index ||
                        playingCollection != currentCollection) &&
                        "text-gray-300",
                      playingSong == index &&
                        playingCollection == currentCollection &&
                        "text-blue-200 md:text-lg",
                    )}
                  >
                    {song.name}
                  </p>
                  ...
                  <div
                    class={cx(
                      "hidden sm:flex justify-center items-center w-4 h-full text-blue-500 text-3xl",
                    )}
                  >
                    {#if song.fav}
                      <p
                        class={cx(
                          "h-2  -translate-y-[1.00rem]",
                          playingSong == index &&
                            playingCollection == currentCollection &&
                            "text-blue-400",
                        )}
                  ...
                  <p
                    class={cx(
                      "text-sm pl-2 text-gray-500 hidden sm:block",
                      playingSong == index &&
                        playingCollection == currentCollection &&
                        "text-slate-200",
                    )}
          ...
          <div
            class="overflow-clip relative rounded-br-xl border-r-2 border-b-2 border-slate-900 bg-slate-900/10 grow max-h-[calc(100vh-31rem)] min-[105rem]:min-h-110"
          >
            <div
              class="absolute right-0 top-80 w-full h-80 bg-gradient-to-t from-slate-900/15 to-slate-900/10"
            ></div>
            ...
            <div
              class="absolute hidden min-[105rem]:block -right-5 top-30 -scale-x-100"
              style="contain: layout; width: {figureWidth /
                2.5}px; height: {figureHeight /
                2.5}px; filter: url(#figsketch);"
            >
              <div
              ...
              >
                ...
                <div
                  style="width: 300%;"
                  class="absolute top-0 left-0 h-full bg-gradient-to-tr to-blue-200/10 from-0% to-70% from-slate-600 mix-blend-overlay"
                ></div>
                <div
                  style="width: 300%;"
                  class="absolute top-0 left-0 h-full bg-gradient-to-tr to-sky-900/10 from-10% to-30% mix-blend-multiply from-sky-900/60"
                ></div>
              </div>
            </div>

            <div
              class="hidden min-[105rem]:flex absolute top-0 left-0 items-center w-full min-h-110"
            >
              <div
                class={cx(
              ...
              >
                <div
                  class={cx(
                    ...
                    characterhover &&
                      "w-[24rem] h-fit border-slate-600 font-sans",
                  )}
                >
                  <p
                    class={cx(
                      "absolute mt-[-0.9rem] translate-x-[-0.1rem] h-full translate-y-1/2 w-full transition-[opacity] duration-300 text-xs align-middle text-center font-black",
                  ...
                  >
                    {#if currentDialogue !== null}
                      {#if currentDialogue?.media?.length > 0}
                        <div
                          class="flex relative z-0 gap-1 mx-auto w-full h-40"
                        >
                          <button
                            onclick={() => {
                              emblaApi?.scrollPrev();
                            ...
                            class={cx(
                              ...
                              emblaApi !== null &&
                                currentSlide !== 0 &&
                                "max-w-6 min-w-6 pl-1",
                              (emblaApi === null || currentSlide === 0) &&
                                "max-w-0 min-w-0",
                            )}
                          >
                          ...
                          >
                            <div class="flex items-start w-full h-full">
                              {#each currentDialogue.media as src, index}
                                <div
                                  class="flex-[0_0_100%] h-full min-w-0 relative w-full"
                                >
                                  <div
                                    class={cx(
                                  ...
                                  >
                                    ...
                                    >
                                      <img
                                        src={concertsMap[src.img]}
                                        alt={name}
                                        class="object-cover w-full h-full duration-200 group-hover:opacity-80"
                                      />
                                    </a>
                                  </div>
                                </div>
                            ...
                            class={cx(
                              ...
                              emblaApi !== null &&
                                emblaApi.selectedScrollSnap() !==
                                  currentDialogue.media.length - 1 &&
                                "max-w-6 min-w-6",
                              (emblaApi === null ||
                                emblaApi.selectedScrollSnap() ===
                                  currentDialogue.media.length - 1) &&
      ...
      <div
        class="absolute block min-[105rem]:hidden -right-5 -bottom-2 -scale-x-100"
        style="contain: layout; width: {figureWidth /
          2.5}px; height: {figureHeight / 2.5}px; filter: url(#figsketch);"
      >
        <div
          class={cx("absolute top-0 left-0 z-20 w-full h-full")}
          style="
      ...
      <div
        class="flex flex-col min-[105rem]:hidden absolute bottom-33 right-35 items-center w-full min-h-110"
      >
        <div
          class={cx(
            "absolute origin-bottom right-0 bottom-0 transition-transform duration-700 ease-[cubic-bezier(0.054,0.564,0.23,1))]",
            characterhover &&
              "sm:translate-y-20 translate-x-20 sm:-translate-x-5 -translate-y-20",
            !characterhover && "",
          )}
        >
          <div
    ...
    <div
      class={cx(
        "p-0 -mt-4 w-full bg-zinc-950/50 rounded-md select-none flex items-start transition-all duration-900 ease-[cubic-bezier(0,1,0,1)]",
        currentMenu === "edits" ||
          currentMenu === "games" ||
          currentMenu === "anime"
          ? "translate-y-0 opacity-100 h-[calc(100vh-15rem)]"
          : "translate-y-30 opacity-0 h-0",
      )}
    >
      <div class="w-full max-w-7xl h-full">
        <div
          class="flex flex-col relative justify-center items-center w-full h-full border-2 border-gray-800 rounded-lg bg-radial-[at_50%_0%] to-150% from-blue-400/10 to-stone-950/70"
        >
          <h2 class="font-mono text-8xl font-black">WIP</h2>
          <p class="-mt-2 -translate-x-8 text-zinc-600">coming soon...</p>
          <div
            class="absolute top-0 left-0 w-full h-full overflow-clip mix-blend-multiply opacity-4"
          >
            <div
              class="overflow-hidden relative w-full h-full rounded-xl scale-300"
            >
              <div
                class="absolute top-0 left-0 w-full h-full opacity-10 grain z-9999"
              ></div>
            </div>
          </div>
          <button
            ...
            class="inline-flex absolute top-3 left-4 gap-2 items-center z-9999 text-slate-400 hover:text-slate-300"
          >
            <Fa icon={faArrowLeft}></Fa>
            back
          </button>
    ...
    {#if currentMenu === "main"}
      <div
        class={cx(
          "p-0 -mt-4 w-full h-[calc(100vh-12rem)] flex items-start transition-[opacity,scale] origin-left duration-700 delay-300 ease-[cubic-bezier(0.115,0.813,0,1)]",
          currentSection !== "" && "opacity-0",
        )}
      >
        <div
          class="flex relative flex-col items-stretch w-full h-full overflow-clip rounded-xl"
        >
          <div
          ...
          <p
            class="hidden absolute right-10 bottom-20 text-sm text-right text-white md:block"
          >
            creative works <br />i've been inspired by.
          </p>

          <div
            class="absolute top-0 left-0 w-full h-full overflow-clip rounded-2xl opacity-50 mix-blend-soft-light"
          >
            <div
              class="overflow-hidden relative w-full h-full rounded-xl scale-140"
            >
              <div
                class="absolute top-0 left-0 w-[110%] h-[110%] z-9999 grain"
              ></div>
            </div>
          </div>
            ...
            >
              <div
                class={cx(
                  ...
                  currentSection == text &&
                    "w-full scale-y-700 duration-[0.3s] ease-[cubic-bezier(0,0.5,0.5,1)]",
                )}
              ></div>
              ...
              <h1
                ...
                class={cx(
                  ...
                  currentSection == text &&
                    "scale-y-800 duration-[0.09s] ease-[cubic-bezier(0,0.5,0.5,1)] scale-x-[3]",
              ...
              <div
                style={`animation-delay: ${delay}ms`}
                class={cx("absolute bg-zinc-950 h-full block-anim z-11")}
              ></div>
            </button>
          {/snippet}
      ...
      <p
        class="left-10 bottom-20 pt-4 pl-1 text-sm text-left text-slate-800 hover:text-slate-600 transition-color duration-400"
      >
        Please note, the music player simply streams using official youtube
        music embeds. I do NOT host copyrighted material.
      </p>
    {/if}
  </div>
</div>
...
{#if currentMenu === "music"}
  <div
    class="flex sticky bottom-5 z-10 justify-center -mb-10 w-full mt-30 md:bottom-9 h-25"
  >
    <div class="px-8 space-y-5 w-screen max-w-7xl">
      <div
        class="flex relative flex-col gap-4 justify-center items-center px-5 w-full h-full rounded-lg border-2 bg-zinc-900 border-zinc-800"
      >
        <div class="flex gap-4 justify-center items-center w-full">
          <div class="md:hidden basis-4/5"></div>
          <div
            class="hidden gap-3 py-2 w-full md:flex text-zinc-500 basis-4/5 text-nowrap"
          >
            {#if playerReady && playingCollection !== null && playingSong != null}
              Now playing:
              <div class="w-full font-mono max-w-100 now-playing-mask">
                <p class="playing-slide">
                  {music_data.collections[playingCollection]?.tracklist[
                    playingSong
                  ].name} on
                  <span class="italic"
                    >{music_data.collections[playingCollection]?.name}</span
                  >
                </p>
              </div>
            {/if}
          ...
          <div class="flex gap-4 justify-center items-center">
            <button
              class="w-6 h-8 text-xl cursor-pointer group"
              onclick={playPrev}
            >
              <Fa
                icon={faBackwardStep}
                class="w-full h-full text-center align-middle duration-200 text-zinc-300 group-hover:text-zinc-50 group-hover:scale-110"
              ></Fa>
            </button>
            <button
              class={cx(
            ...
            >
              {#if playerReady}
                {#if playerState === YT.PlayerState.PAUSED || playerState === YT.PlayerState.UNSTARTED || playerState === YT.PlayerState.ENDED || playerState === YT.PlayerState.CUED}
                  <Fa
                    icon={faPlay}
                    class="w-full h-full text-xl text-center align-middle"
                  ></Fa>
                {:else if playerState === YT.PlayerState.PLAYING}
                  <Fa
                    icon={faPause}
                ...
                {:else if playerState === YT.PlayerState.BUFFERING}
                  <p
                    class="text-5xl h-full font-[Arial] animate-spin origin-[50%_42.0%]"
        ...
        <div class="flex gap-5 items-center w-full text-gray-300">
          ...
          {/if}
          <div
            class="flex relative items-center w-full h-2 rounded-sm bg-zinc-800"
          >
            <input
              type="range"
              min="0"
              max={music_data.collections[playingCollection]?.tracklist[
                playingSong
              ].length}
              step="any"
              oninput={(event) => {
            ...
            />
            <div
              class="h-full pointer-events-none absolute left-0 rounded-sm ease-[cubic-bezier(0.5,0.5,0.5,0.5)] transition-all duration-50 bg-slate-200"
              style={`width: ${100 * Math.max(Math.min(currentTime / music_data.collections[playingCollection]?.tracklist[playingSong].length, 1), 0)}%`}
            ></div>
            <div class="absolute w-full h-full pointer-events-none">
              <div class="relative w-full h-full">
                {#each music_data.collections[playingCollection]?.tracklist[playingSong].hotspots as spot}
                  <div
                    class="absolute w-1 h-2 bg-zinc-950/20"
                    style={`left: ${(100 * spot.time) / music_data.collections[playingCollection]?.tracklist[playingSong].length}%;`}
                  ></div>
                {/each}
              </div>
            </div>
        ...
        <div class="hidden absolute right-4 bottom-20">
          <img
            src={sittingSketch}
            class="hidden w-55 -scale-x-100"
            style=" filter: url(#figsketch);"
          />
        </div>
      </div>
    </div>
...
<style>
  ...
  .fly-in {
    animation: fly-in-a 2s cubic-bezier(0, 0.5, 0, 1) both;
  }
  ...
  .fade-in {
    animation: fade-in-a 2s cubic-bezier(0, 0.5, 0, 1) both;
  }
