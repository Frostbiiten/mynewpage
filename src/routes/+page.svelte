<svelte:head>
  ...
  <meta property="og:description" content="Welcome to my site! Visit the projects page to view my past works."/>
  ...
  <meta name="twitter:description" content="Welcome to my site! Navigate to Projects to view my past works."/>
...
<script>
    ...
    import mobileBg from "$lib/img/mobilebg.webp"
    ...
    const animate = () => {
      if (isMobile)
      {
        smoothX += (targetX - smoothX) * 0.9;
        smoothY += (targetY - smoothY) * 0.9;
      }
      else
      {
  ...
  import { mountBunnyScene } from '$lib/gfx/scenes/bunnyScene.js';
...
<div class="hidden absolute z-0 justify-center -mt-10 w-full overflow-clip select-none h-250 md:flex">
...
<div class="flex flex-col items-center -mt-5 w-full">
  <div class="flex flex-col gap-5 justify-center items-center px-8 w-full max-w-7xl md:gap-7">
    <div class="z-2 img relative w-full md:h-[40rem] rounded-md md:rounded-xl select-none border-2 border-slate-800/40 overflow-clip bg-clip-padding">
      ...
      <div
        class="absolute top-0 left-0 w-full h-full bg-top bg-no-repeat bg-cover transition-all duration-200 saturate-110 aspect-auto md:hidden",
        style="transform: perspective(18rem) translate3d(-0rem, -1rem, 5rem) rotateY({-smoothX * 4}deg) rotateX({smoothY * 4}deg); background-image:url({mobileBg}) ">
      </div>
      ...
      <div class="top-0 left-0 p-7 w-full h-full md:absolute md:p-12">
          ...
          {#each { length: 12 }, i}
            <div class="absolute top-10 left-10 w-0 h-0 duration-700 animate-spin-slow">
                <div
                  ...
                  >
                    <div
                    class="w-36 h-36 rounded-full md:border-blue-900/90"
        ...
        >
          ...
          {#each { length: 7 }, i}
          <div class="absolute right-0 bottom-0 w-0 h-0 py-45 px-45 md:p-56 animate-spin-slow-2">
            <div
              class="origin-top-left w-[10rem] h-[20rem]"
              style="transform: scale({1 - dist01 * 0.01}) rotate({i / 7 * 360}deg);">
              <div
              class="w-[8rem] h-[8rem] text-[14rem] font-bold translate-y-[100px] md:translate-y-[340px]">
                <div class="w-100 h-100 md:fill-[rgba(240,220,160,1)] fill-slate-100">
        ...
        >
          ...
          <div class={cx("absolute translate-x-[-60%] right-0 bottom-0 p-12 rounded-[3rem] outline-black -scale-x-150 scale-y-150")}>
            ...
            <div class={cx(characterhover && "boop-anim", "relative")} style="width: {figureWidth}px; height: {figureHeight}px; filter: url(#figsketch);">
        ...
        <div
          ...
          class="absolute hidden md:block z-[15] origin-bottom-right inset-0 h-[120%] w-[120%] bg-transparent bg-[linear-gradient(to_right,rgba(0,0,0,0.1)_1px,transparent_1px),linear-gradient(to_bottom,rgba(0,0,0,0.1)_1px,transparent_1px)] bg-[size:30px_30px] md:bg-[linear-gradient(to_right,rgba(128,128,128,0.07)_1px,transparent_1px),linear-gradient(to_bottom,rgba(128,128,128,0.07)_1px,transparent_1px)] md:bg-[size:24px_24px]">
        </div>
    ...
    <p class={cx("w-full pl-1 italic text-center hidden md:block md:text-left transition-colors", infoText ? "md:text-slate-400 text-stone-400" : "md:text-slate-500 text-stone-500")}>
      {#if infoText}
        {infoText}
      {:else}
        Welcome to <span class="font-semibold">my world!</span> Feel free to click around and explore. 
      {/if}
    </p>
    </div>
  </div>
