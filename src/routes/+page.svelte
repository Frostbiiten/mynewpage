<script>
  ...
  onMount(() => {
    // Set browser to true once we're running in the browser
    browser = true;
...
<style>
  ...
  .plus-pattern {
  --size: 32px; /* size of grid cell */
  background:
    repeating-linear-gradient(
      0deg,
      transparent,
      transparent var(--size),
      rgba(0, 0, 0, 0.1) var(--size),
      rgba(0, 0, 0, 0.1) calc(var(--size) + 1px)
    ),
    repeating-linear-gradient(
      90deg,
      transparent,
...
.plus-pattern::before {
  ...
  -webkit-mask-image: repeating-radial-gradient(circle at center, white 1px, transparent 1px, transparent var(--size));
  mask-image: repeating-radial-gradient(circle at center, white 1px, transparent 1px, transparent var(--size));
  width: 100%;
  height: 100%;
  display: block;
}
...
<div class="flex flex-col gap-7 justify-center items-center px-8 w-full">
    ...
    <div class="grain img relative max-w-7xl w-full h-[40rem] rounded-xl select-none border-2 border-amber-600/20 overflow-clip">
      <div
        class="absolute top-0 left-0 p-12 w-full h-full"
        >


        <div
          class="absolute left-0 top-0 min-w-7xl scale-[1] image"
          style="transform: translate3d({smoothX * -10}px, {smoothY * -5 - 450}px, 0) scale({1 + dist01 * 0.01});"
        >
          <img
            class="absolute top-0 left-0 z-1 min-w-7xl"
            src={bg}
            alt="Parallax"
            style="filter: blur({easeInOutCubic(1-dist01) * 8}px) contrast(1.07) hue-rotate(25deg) brightness(1.0);"
          />
          <img
            class="absolute z-[2] min-w-7xl top-0 left-0"
            src={bgbokeh}
            alt="Parallax"
            style="opacity: {easeInOutCubic(1-dist01)}; filter: contrast(1.07) hue-rotate(25deg) saturate(0.9) brightness(1.0);"
          />
        </div>


        <div class="absolute top-0 left-0 z-10 w-full h-full" style="filter: blur({easeInOutCubic(Math.abs(1- dist01)) * 4}px);">
          {#each { length: 12 }, i}
          <div class="absolute top-10 left-10 z-10 w-0 h-0 animate-spin-slow">
                <div
                  class="z-10 origin-top-left w-[10rem] h-[20rem]"
                  style="transform: rotate({i / 12 * 360}deg);"
                  >
                    <div
                    class="w-36 h-36 rounded-full border-orange-400/30"
        ...
        <div
          class="absolute top-0 left-0 z-10 w-full h-full mix-blend-difference origin-right"
          style="filter: blur({easeInOutCubic(Math.abs(1 - dist01)) * 2}px); transform: scale({(1 + easeOutCubic(dist01) * -0.01)});"
        >
          {#each { length: 10 }, i}
          <div
          class="absolute right-0 bottom-0 z-10 px-56 py-56 w-0 h-0 animate-spin-slow-2">
                <div
                  class="z-10 origin-top-left w-[10rem] h-[20rem]"
                  style="transform: scale({1 - dist01 * 0.01}) rotate({i / 10 * 360}deg);">
                    <div
                    class="w-[8rem] h-[8rem] text-orange-400/90 text-[14rem] font-bold"
                    style="transform: translate3d(0px, -340px, 0); ">
                      <div  class="z-[2] w-96 h-96">
        ...
        <div
          class="absolute top-0 left-0 z-10 w-full h-full origin-bottom-right"
          style="
            transform: translate3d({smoothX * -55 - 114}px, {smoothY * -13 + 30}px, 0) scale({0.4 * (1 - dist01 * 0.02)});
            filter: blur({easeInOutCubic(dist01) * 6}px)"
        >
          <div class="absolute right-0 bottom-0">
                <div style="position: relative; width: {figureWidth}px; height: {figureHeight}px;">
                  <div
                  ...
                  >
                    <img
                      ...
                      style="
                        ...
                        object-fit: cover;
                        display: block;
                        filter: contrast(1.07) saturate(0.1) hue-rotate(190deg) brightness({1.8 - dist01 * 0.8});
                      "
                    />
                    <div
                    style="width: 300%; opacity: {0.3 +dist01 * 0.7}; transform: translate3d({smoothX * 200 - 100}px, {smoothY * -13 + 30}px, 0)"
                    class="absolute top-0 left-0 h-full bg-gradient-to-tl to-orange-400 from-10% to-100% mix-blend-overlay from-red-900/70"></div>
                    <div
                    style="width: 200%; opacity: {0.7 - dist01 * 0.3}; transform: translate3d({smoothX * 200 - 100}px, {smoothY * -13 + 30}px, 0)"
                    class="absolute top-0 left-0 h-full bg-gradient-to-tl to-amber-300 from-10% to-90% mix-blend-overlay  from-red-900"></div>

                  </div>

                  <div class="absolute top-0 left-0 z-30 w-full h-full mix-blend-multiply">
                    <div
                      class="absolute top-0 left-0"
                      style="
                        opacity: {1 - dist01 * 0.9};
                        background: linear-gradient(
                          -35deg,
                          rgba(28, 28, 36, 0.4) {30 + dist01 * -10}%,
                          #e3bf94 {32 + dist01 * -10}%,
                          #fee2e2 {35 + dist01 * -10}%
                        );
                        width: 100%;
        ...
        <div
          style="transform: translate3d({smoothX * -55 * 0.4}px, {smoothY * -13 * 0.6}px, 0) scale({1.2 * (1 - dist01 * 0.03)});"
        ...
        <div class="flex absolute bottom-0 left-0 z-40 gap-6 items-end p-12 px-16 w-full text-2xl text-orange-400/70">
          <a
          href="https://github.com/Frostbiiten" target="_blank" rel="noopener noreferrer" class="transition-all duration-200 hover:text-amber-100">
          <Fa icon={faGithub}/>
          </a>
          
          <a
          href="https://www.linkedin.com/in/edem-hoggar/" target="_blank" rel="noopener noreferrer" class="transition-all duration-200 hover:text-amber-100">
            <Fa icon={faLinkedinIn}/>
          </a>

          <a href="https://www.linkedin.com/in/edem-hoggar/" target="_blank" rel="noopener noreferrer" class="transition-all duration-200 hover:text-amber-100">
            <Fa icon={faAt}/>
          </a>

          <a href="https://x.com/frostbiiten" target="_blank" rel="noopener noreferrer" class="transition-all duration-200 hover:text-amber-100">
            <Fa icon={faTwitter}/>
          </a>
        </div>

        <div class="relative z-40 select-none">
          ...
          <a href="https://cs.uwaterloo.ca/" target="_blank" rel="noopener noreferrer"
            class="flex z-40 flex-row gap-2 items-center px-1 w-56 text-sm transition-all text-amber-500/60 group-hover:text-amber-500 group">
          <span class="duration-200 transition-color group-hover:text-amber-500">@ University of Waterloo</span> <Fa class="duration-200 scale-90 group-hover:scale-100" icon={faSquareArrowUpRight}></Fa>
          </a>

          <div class="py-3 tracking-wider leading-9 font-mono px-4 mt-6 w-124 text-orange-200 rounded-sm bg-zinc-950/[0.0] h-60">
    ...
    <div class="grain opacity-0 relative max-w-7xl w-full h-[40rem] select-none border-2 border-amber-200/10 overflow-clip ">

        <div
          style="transform: translate3d({smoothX * -5}px, {smoothY * -5 + scroll * 0.2}px, 0) scale(1.8)"
          class="absolute z-[15] origin-bottom-right inset-0 h-full w-full bg-[linear-gradient(to_right,rgba(128,128,128,0.07)_1px,transparent_1px),linear-gradient(to_bottom,rgba(128,128,128,0.07)_1px,transparent_1px)] bg-[size:12px_12px]">
        </div>
    </div>
