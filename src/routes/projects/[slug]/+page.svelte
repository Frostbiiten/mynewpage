<script>
  ...
  let project_data = $state(
    project_data_raw.projects.find((el) => el.name === data.slug),
  );
...
<svelte:head>
  ...
  <meta
    name="description"
    content={`${data.slug} from ${getDateInfo(project_data?.time)}`}
  />
  ...
  <div class="px-4 -mt-5 space-y-4 max-w-screen w-7xl">
      ...
      <div class="py-1 pl-4 space-y-4 h-min">
        <div class="space-y-1">
          ...
          <h1
            ...
            class="h-full font-mono text-5xl md:text-6xl lg:text-7xl"
          >
            {#each Array.from(data.slug) as char, index}
              <span
                class="inline-block animate-text-reveal"
                style="animation-delay: {index * 0.03 -
                  0.6}s; animation-fill-mode: backwards;"
              >
                {@html char === " " ? "&nbsp;" : char}
              </span>
            {/each}
        ...
        >
          ...
          {#if "src" in project_data}
            <a
              href={project_data.src}
              target="_blank"
              rel="noopener noreferrer"
              class="flex flex-row gap-2 items-center px-3 w-min text-sm font-bold align-middle rounded-lg transition-all duration-200 text-zinc-950 bg-slate-300 hover:bg-slate-500"
            >
              Source
              <Fa
                class="justify-center h-full translate-y-[-0.05rem] text-md"
    ...
    {#if "embed" in project_data}
      <div class="space-y-2">
        <div
          id="unityContainer"
          class="max-w-7xl aspect-[3/2] border-2 border-slate-700"
        >
          <iframe
            id="unityIframe"
            title="Game Embed"
    ...
    {#if "galleryCount" in project_data}
      <Gallery
        project={data.slug}
        imgCount={project_data.galleryCount[0]}
        vidCount={project_data.galleryCount[1]}
      ></Gallery>
      <div class="h-30"></div>
    {/if}
  </div>
